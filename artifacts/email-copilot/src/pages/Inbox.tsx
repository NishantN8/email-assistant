    ...
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="shrink-0 px-6 py-3 border-b border-border/50 flex items-center justify-between bg-background/80 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-1">
          <button onClick={onReply} className="p-2 rounded-lg hover:bg-secondary text-muted-foreground transition-colors" title="Reply (r)">
            <Reply className="w-4 h-4" />
          </button>
          <button onClick={onArchive} className="p-2 rounded-lg hover:bg-secondary text-muted-foreground transition-colors" title="Archive (e)">
            <Archive className="w-4 h-4" />
          </button>
          <button className="p-2 rounded-lg hover:bg-secondary text-muted-foreground transition-colors" title="Forward">
            <Forward className="w-4 h-4" />
          </button>
          <button className="p-2 rounded-lg hover:bg-secondary text-muted-foreground transition-colors" title="More">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-foreground mb-3">{email.subject}</h2>
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            <div className="w-9 h-9 rounded-full bg-secondary flex items-center justify-center font-bold text-foreground text-sm shrink-0">
              {email.from?.charAt(0)?.toUpperCase() ?? "?"}
            </div>
            <div>
              <div className="font-medium text-foreground">{email.from}</div>
              <div className="text-xs">{formatTimeAgo(email.receivedAt)}</div>
            </div>
          </div>
        </div>
        <div className="prose prose-sm prose-invert max-w-none">
          <div className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">{email.body || email.snippet}</div>
        </div>
        {/* Inline ReplyBox */}
